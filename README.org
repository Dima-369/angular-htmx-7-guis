...
The backend uses Spring Boot 3.1.5, Java 17, Gradle, Spring Web, Thymeleaf 3.1.2 and starts on port 8083 when launched. This was created via https://start.spring.io/
...
#+begin_src bash
docker build -t 7guis . && docker run --publish 8083:8083 7guis
#+end_src
